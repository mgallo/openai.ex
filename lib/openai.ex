defmodule OpenAI do
  ...
  def assistants_modify(assistant_id, params, config \\ %Config{})
      when is_bitstring(assistant_id) and is_list(params) and is_struct(config) do
  ...
  def assistants_delete(assistant_id, config \\ %Config{})
      when is_bitstring(assistant_id) and is_struct(config) do
  ...
  def assistant_file(assistant_id, file_id, config \\ %Config{})
      when is_bitstring(assistant_id) and is_bitstring(file_id) and is_struct(config) do
  ...
  **NOTE:** At the time of this writing this functionality remains undocumented by OpenAI.
  ...
  def threads_modify(thread_id, params, config \\ %Config{})
      when is_bitstring(thread_id) and is_list(params) and is_struct(config) do
  ...
  def threads_delete(thread_id, config \\ %Config{})
      when is_bitstring(thread_id) and is_struct(config) do
  ...
  def thread_message(thread_id, message_id, config \\ %Config{})
      when is_bitstring(thread_id) and is_bitstring(message_id) and is_struct(config) do
  ...
  def thread_message_modify(thread_id, message_id, params, config \\ %Config{})
      when is_bitstring(thread_id) and is_bitstring(message_id) and is_list(params) and
             is_struct(config) do
  ...
  def thread_message_file(thread_id, message_id, file_id, config \\ %Config{})
      when is_bitstring(thread_id) and is_bitstring(message_id) and is_bitstring(file_id) and
             is_struct(config) do
  ...
  def thread_run(thread_id, run_id, config \\ %Config{})
      when is_bitstring(thread_id) and is_bitstring(run_id) and is_struct(config) do
  ...
  def thread_run_step(thread_id, run_id, step_id, config \\ %Config{})
      when is_bitstring(thread_id) and is_bitstring(run_id) and is_bitstring(step_id) and
             is_struct(config) do
  ...
  ## Example request
  ```elixir
  OpenAI.files_content("file-123321")
  ```
  ...
  def files_content(file_id, config \\ %Config{}),
    do: Files.fetch_content(file_id, config)
